...
const CarComponent = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (bookedDates[selectedDate] && bookedDates[selectedDate].includes(selectedCar)) {
      alert('The selected car is already booked for the selected date. Please choose another date or car.');
      return;
    }
    if (!/^\d+$/.test(phone)) {
    ...
    try {
      ...
      setBookedDates({ ...bookedDates, [selectedDate]: [...(bookedDates[selectedDate] || []), selectedCar] });
